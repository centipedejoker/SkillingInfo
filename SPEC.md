...
**Document status:** v9 — Reviewed, hardened, prior-art-checked, scope-decided, deployment-ready, first-playtested & Phase-2-implemented specification (2026-08-11)
...
>
> **Changelog from v7 (v8):** Three defects of one shape, all found by driving `SessionManager` through real tick sequences rather than by reading it — the tick-ordering layer had no tests at all, so 62 green ones said nothing about it. (1) §18: the item-flow pipeline ends in two catch-alls that credit *anything* unexplained inside the XP window, so every pool feeding them must be drained unconditionally and claimed against in a fixed order. Unequipping was booked as skilling output (the wield direction was handled, the mirror was not) and bank withdrawals likewise (deposits were capped by §25a's three-way minimum, withdrawals had no counterpart); an equip outside the XP window sat in an undrained pool and silently cancelled the next consumption of that item. (2) §37: kill counting was the one signal gated on ACTIVE alone, and its paused branch advanced the task baseline past the kill on the way out — so kills during a pause were lost rather than deferred, while loot from the same kill was credited and did resume the session. (3) §13a (new): a manual pause is an instruction, not an inference, so it is sticky and records nothing; auto-pause is unchanged. Sections 13a (new), 18, 25a, 37, 56 updated, marked `[v8]`.
>
> **Changelog from v8 (v9):** Adversarial pre-submission review; fifteen defects, thirteen reproduced with tests before being fixed. The item-flow accounting took most of it. §18's "container transfers undercount rather than invent gain" was withdrawn as wrong on both counts — a coal-bag trip read `RETAINED 0.0%` for a fully banked load, and a figure floored at zero is a different claim rather than a smaller one; containers RuneLite exposes are now claimed, and §50's `OTHER_INVENTORY_LOSS` is resurrected for the bags it doesn't, gated on whether the activity has inputs at all. Noted items broke both directions of bank correlation, since the two halves of one movement carry different ids. §25a now separates what a deposit *explains* from what it may *credit*. §33's confidence ordering was being read as a quantity preference, dropping everything not yet banked from the projection. §5: a stale XP baseline turned an account switch into a +9.5m phantom session, and `LOGGED_IN` was being treated as a login when it fires on every region change. §10: everything earned while the prompt was on screen was discarded, from a session whose `startedAt` claimed that time anyway. §14: a combat session's headline showed the Slayer share alone, an eighth of the truth. §9: any method awarding three tracking groups — barbarian fishing, birdhouse runs — could never start a session. §20a: pickups are now confirmed against ground evidence rather than the click, which finally gives `GroundItemTracker` a reader. §37: `LootReceived` comes from the Loot Tracker *plugin*, not core, and a comment in the source said otherwise. §48a (new) writes down the thread ownership that two of these came out of. Sections 5, 9, 10, 13, 14, 18, 20a, 25a, 33, 37, 44, 48a (new), 50 updated, marked `[v9]`.
